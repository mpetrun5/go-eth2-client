...
func TestSyncCommittee(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Current",
			opts: &api.SyncCommitteeOpts{State: "head"},
		},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			response, err := service.(client.SyncCommitteesProvider).SyncCommittee(ctx, test.opts)
			require.NoError(t, err)
			require.NotNil(t, response)
			require.NotNil(t, response.Data)
			require.True(t, len(response.Data.Validators) > 0)
		})
	}
}
