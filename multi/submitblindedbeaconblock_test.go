...
func TestSubmitBlindedBeaconBlock(t *testing.T) {
	...
	for i := 0; i < 128; i++ {
		err := multiClient.(consensusclient.BlindedBeaconBlockSubmitter).SubmitBlindedBeaconBlock(ctx, &api.VersionedSignedBlindedBeaconBlock{})
		require.NoError(t, err)
	}
